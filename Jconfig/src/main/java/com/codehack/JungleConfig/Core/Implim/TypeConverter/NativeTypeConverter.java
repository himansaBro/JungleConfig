package com.codehack.JungleConfig.Core.Implim.TypeConverter;
...
public class NativeTypeConverter implements TypeConverter {
    ...
    public NativeTypeConverter(CacheInterface manager, boolean FALLBACK_TO_SERIALIZE,
            TypeConverterAdapter... Adapters) {
    ...
    private Map.Entry<String, String> ConvertToSave(Object dataObj, String type) {
        if (dataObj == null)
            return Map.entry("null", "null"); // Handle null values safely
        if (type.equals("Json"))
            return Map.entry("Json", JJacksonModule.ConvertToJson(dataObj));
        if (type.isBlank())
            type = dataObj.getClass().getSimpleName();
