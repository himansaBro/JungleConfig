...
public class NativeCache implements CacheInterface {
    ...
    public NativeCache(ConverterInterface converterInterface, int CACHE_WRITE_INTERVAL, int CACHE_READ_INTERVAL,
            boolean autoCommit, boolean useCache) {
    ...
    private void updateCache() {
        if (((Interations > CACHE_WRITE_INTERVAL) || isCacheDirty || (readInterations > CACHE_READ_INTERVAL))
                && AutoCommit) {
    ...
    private TypeMap<String, String, String> Read() {
        if (!useCache)
            return converterInterface.decode();
        if (!AutoCommit)
            return midgradeSnapShot(saveQuery);
        updateCache();
        readInterations++;
        return cache;
    }
    ...
    @Override
    public TypeMap<String, String, String> query(boolean iKey, String keyReg, boolean iType, String typeReg,
        ...
        for (TypeMap.Entry<String, String, String> dd : Read().getEntryList()) {
            if ((iKey ^ dd.getKey().matches(keyReg)) && (iType ^ dd.getValue1().matches(typeReg))
                    && (iVal ^ dd.getValue2().matches(valReg))) {
                outData.put(dd);
            }
        }
        return outData;
